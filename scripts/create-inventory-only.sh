...
# Script to create only product inventory with fixed ID range
# ID format: 68f1f642023c5a0cd694xxxx (where xxxx is 4 hex digits: 0000-ffff)

# Configuration
API_URL="${API_URL:-http://localhost:80/api/product-inventory}"
PREFIX="68f1f642023c5a0cd694"
START_INDEX="${START_INDEX:-0}"
END_INDEX="${END_INDEX:-65535}"
MIN_STOCK="${MIN_STOCK:-1000}"
...
echo "Stock Range: $MIN_STOCK - $MAX_STOCK"
echo ""
...
for ((i=$START_INDEX; i<$END_INDEX; i++)); do
    ...
    STOCK=$((RANDOM % (MAX_STOCK - MIN_STOCK + 1) + MIN_STOCK))
...
EOF
    ...
    if [ "$HTTP_CODE" -eq 201 ] || [ "$HTTP_CODE" -eq 200 ]; then
        echo -e "${GREEN}✓${NC} [$((i-START_INDEX+1))/$((END_INDEX-START_INDEX))] $PRODUCT_ID - Stock: $STOCK"
        ((SUCCESS_COUNT++))
    else
